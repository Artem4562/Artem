...
void WindowFullInformation(std::vector<float> *Iay,std::vector<float> *Time) {
    ...
    if (ImPlot::BeginPlot("Graph Ia, Ib, Ic, In", SizeGraph)) {
...
int main(int arc, char** args) {
    ...
    std::vector<float> IAMY;
    std::vector<float> TIMEX;
        ...
        while(window_flag){
            ...
            if (Y.size()){
                ...
                if (Y.size()>0){
                    if(X[i]>X[i-1]){
                        razn = int(X[i]-X[i-1]);
                        std::this_thread::sleep_for(std::chrono::microseconds(razn*100));
                    }
                    else {
                        razn = int(X[i]+X[i-1]-1);
                        std::this_thread::sleep_for(std::chrono::microseconds(razn*100));
                    }

                }
                if(i>0) std::transform(TIMEX.begin(), TIMEX.end(), TIMEX.begin(), [razn](float &c){ return c-razn; });
            }
            ...
            if(IAMY.size()>400 || TIMEX.size()>400){
                ...
                IAMY.erase(begin1 , end1 - 200);
                TIMEX.erase(begin2 , end2 - 200);
            }
    ...
    std::thread t3 ([&](){
                            ...
                            )) == NULL)
        {
            ...
            if((res = pcap_compile(fp, &fcode, "not udp and ether dst 01.0c.cd.04.00.01", 1, 0)) < 0) //составление фльтра 
                {	
