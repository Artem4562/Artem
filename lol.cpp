...
int main(int, char**) {
    ...
    std::thread t2 ([&](){
        ...
        while(window_flag){
            ...
            IAMY.push_back(Y[i]);
            TIMEX.push_back(0);
            std::this_thread::sleep_for(std::chrono::milliseconds(int(X[i]-X[i-1])));
            if(i>0) std::transform(TIMEX.begin(), TIMEX.end(), TIMEX.begin(), [X,i](float &c){ return c-(X[i]-X[i-1]); });
            //int(X[i]-X[i-1])
            
            //std::cout<<IAMY.size()<<'\t'<<i<<'\n';
